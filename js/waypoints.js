...
function buildPoints() {
    ...
    for (var i = 0; i < points.length; i++) {
        ...
        markers.push(marker);
        marker.setMap(map);
        html += "<tr><td>" + points[i].LocationName + "</td><td><input type=\"text\" name=\"latitudes[]\" value=\"" + roundNumber(points[i].LatLng.lat(), 6) + "\"></td><td><input type=\"text\" name=\"longitudes[]\" value=\"" + roundNumber(points[i].LatLng.lng(), 6) + "\"></td><td><button class=\"delete btn\" onclick=\"removeRow(" + i + ");\">Eliminar</button></td><td>";
        if (i < points.length - 1) {
            html += "<button class=\"moveDown btn\" onclick=\"moveRowDown(" + i + ");\">Mover Abajo</button>";
        }
        html += "</td><td>";
        if (i > 0) {
            html += "<button class=\"moveUp btn\" onclick=\"moveRowUp(" + i + ");\">Mover Arriba</button>";
        }
        html += "</td></tr>";
    }
